...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    td.truncated { max-width:280px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; cursor:pointer; }
    td.truncated.expanded { white-space:pre-wrap; word-break:break-word; overflow:visible; }
...
<body>
  <div class="page">
    ...
    <div class="card" style="margin-top:16px;">
      ...
      <div class="toolbar">
        ...
        <div class="field"><label for="clientIpContains">Client IP contains</label><input id="clientIpContains" placeholder="185.23.44.10"></div>
  ...
  <script>
    ...
    function renderRows(rows){ const body = q('rowsBody'); if(!rows.length){ body.innerHTML = '<tr><td colspan="14" class="muted">No rows matched the current filters.</td></tr>'; return; } body.innerHTML = rows.map(row => `<tr><td>${escapeHtml(row.timestampUtc)}</td><td>${escapeHtml(row.clientIp)}</td><td>${escapeHtml(row.method)}</td><td>${escapeHtml(row.scStatusCode ?? '')}</td><td>${escapeHtml(row.scSubStatusCode ?? '')}</td><td>${escapeHtml(row.scWin32StatusCode ?? '')}</td><td><code>${escapeHtml(row.uriStem)}</code></td><td><code>${escapeHtml(row.uriQuery)}</code></td><td>${escapeHtml(row.host)}</td><td>${escapeHtml(row.timeTakenMs ?? '')}</td><td>${escapeHtml(row.csBytes ?? '')}</td><td>${escapeHtml(row.scBytes ?? '')}</td><td class="truncated" title="${escapeHtml(row.userAgent)}">${escapeHtml(row.userAgent)}</td><td class="truncated" title="${escapeHtml(row.referer)}">${escapeHtml(row.referer)}</td></tr>`).join(''); body.querySelectorAll('td.truncated').forEach(td => td.addEventListener('click', () => td.classList.toggle('expanded'))); }
